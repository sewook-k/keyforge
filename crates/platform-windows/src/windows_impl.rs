...
use crate::{DeviceInventoryError, KeyboardDeviceInfo};
...
pub fn list_connected_keyboards() -> Result<Vec<KeyboardDeviceInfo>, DeviceInventoryError> {
    ...
    for keyboard in &mut keyboards {
        ...
        if let Some(display_name) = metadata.display_name
            && should_replace_display_name(&keyboard.name, &display_name)
        {
...
fn should_replace_display_name(current: &str, candidate: &str) -> bool {
    ...
    !candidate.is_empty()
        && !matches!(candidate, "HID 키보드 장치" | "HID Keyboard Device")
        && !candidate.eq_ignore_ascii_case(current.trim())
}
...
unsafe extern "system" fn keyboard_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    let event = KeyEvent {
        key: vk_name(data.vkCode, data.scanCode, data.flags),
        phase,
        origin,
        repeat: false,
    };
...
mod tests {
    ...
    #[test]
    fn keeps_vid_pid_fallback_when_setupapi_only_offers_generic_hid_name() {
        assert!(!should_replace_display_name(
            "HID 키보드 · VID_2B89 / PID_6209",
            "HID 키보드 장치"
        ));
        assert!(should_replace_display_name(
            "HID 키보드 · VID_2B89 / PID_6209",
