...
use windows_sys::Win32::{
    Devices::{
        ...
        Properties::{
            ...
            DEVPKEY_Device_Model, DEVPKEY_NAME, DEVPROP_TYPE_GUID, DEVPROP_TYPE_STRING,
            DEVPROP_TYPE_STRING_INDIRECT, DEVPROP_TYPE_STRING_LIST, DEVPROPTYPE,
        },
    },
    Foundation::{
...
fn read_metadata(set: HDEVINFO, device: &SP_DEVINFO_DATA) -> PnpKeyboardMetadata {
    ...
    let shell_name = property_string(set, device, &DEVPKEY_NAME);
    let model_name = property_string(set, device, &DEVPKEY_Device_Model);
...
fn preferred_display_name(candidates: &[Option<String>]) -> Option<String> {
    candidates
        .iter()
        .flatten()
        .filter_map(|value| normalize_display_candidate(value))
        .find(|value| !looks_generic_keyboard_name(value))
        .or_else(|| {
            candidates
                .iter()
                .flatten()
...
fn normalize_display_candidate(value: &str) -> Option<String> {
    let value = value.trim();
    (!value.is_empty()).then(|| value.to_owned())
}

fn looks_generic_keyboard_name(value: &str) -> bool {
    matches!(
    ...
    fn chooses_display_name_in_requested_pnp_priority_order() {
        assert_eq!(
            preferred_display_name(&[
                Some("HID 키보드 장치".into()),
                Some("Keychron K2".into()),
        ...
        assert_eq!(
            preferred_display_name(&[Some("HID Keyboard Device".into()), None]).as_deref(),
