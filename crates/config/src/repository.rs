...
mod tests {
    use super::*;
    use crate::{
        ConditionGroup, ConditionOperator, DeviceSelector, MatchCondition, Profile,
        ProfileActivation, ProfileScope, TextOperator,
    };
    ...
    #[test]
    fn version_three_defaults_missing_profile_activation() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("settings.json");
        let repo = SettingsRepository::new(&path);
        let mut value = serde_json::to_value(Settings::default()).unwrap();
        ...
        let loaded = repo.load_or_default().unwrap();
        assert_eq!(loaded.schema_version, crate::CURRENT_SCHEMA_VERSION);
        assert!(loaded.profiles[0].activation.connected_keyboards.is_empty());
    }

    #[test]
    fn saves_connected_keyboard_activation_selectors() {
        let dir = tempdir().unwrap();
        let repo = SettingsRepository::new(dir.path().join("settings.json"));
        let mut settings = Settings::default();
        settings.profiles[0].activation = ProfileActivation {
            connected_keyboards: vec![DeviceSelector {
                ...
                manufacturer_contains: Some("Logitech".into()),
                name_contains: Some("Mechanical".into()),
                is_virtual: Some(false),
            }],
        };

        let saved = repo.save(settings, 0).unwrap();
        let loaded = repo.load_or_default().unwrap();
        assert_eq!(loaded.profiles[0].activation, saved.profiles[0].activation);
    }
