use crate::model::{
    Action, CURRENT_SCHEMA_VERSION, DeviceSelector, ProfileScope, Settings, Trigger,
};
use thiserror::Error;
use uuid::Uuid;
...
pub enum ValidationError {
    ...
    #[error("profile {0} contains an invalid connected keyboard selector")]
    InvalidDeviceSelector(Uuid),
...
pub fn validate(settings: &Settings) -> Result<(), ValidationError> {
    ...
    for profile in &settings.profiles {
        ...
        if profile
            ...
            .connected_keyboards
            .iter()
            .any(|selector| !valid_device_selector(selector))
        {
            return Err(ValidationError::InvalidDeviceSelector(profile.id));
        }
...
fn valid_device_selector(selector: &DeviceSelector) -> bool {
    if selector.is_empty() {
        return false;
    }
    valid_hex_field(selector.vendor_id.as_deref(), 4)
        && valid_hex_field(selector.product_id.as_deref(), 4)
        && valid_hex_field(selector.interface_id.as_deref(), 2)
        && valid_text_field(selector.manufacturer_contains.as_deref())
        && valid_text_field(selector.name_contains.as_deref())
}

fn valid_hex_field(value: Option<&str>, len: usize) -> bool {
    value.is_none_or(|value| {
        value.len() == len && value.chars().all(|character| character.is_ascii_hexdigit())
    })
}

fn valid_text_field(value: Option<&str>) -> bool {
    value.is_none_or(|value| !value.trim().is_empty() && value.trim() == value)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{ProfileActivation, Settings};

    #[test]
    fn rejects_empty_connected_keyboard_selector() {
        let mut settings = Settings::default();
        settings.profiles[0].activation = ProfileActivation {
            connected_keyboards: vec![DeviceSelector::default()],
        };
        let error = validate(&settings).unwrap_err();
        assert!(matches!(
            error,
            ValidationError::InvalidDeviceSelector(profile_id)
            if profile_id == settings.profiles[0].id
        ));
    }

    #[test]
    fn accepts_hex_and_name_based_connected_keyboard_selector() {
        let mut settings = Settings::default();
        settings.profiles[0].activation = ProfileActivation {
            connected_keyboards: vec![DeviceSelector {
                vendor_id: Some("046D".into()),
                product_id: Some("C31C".into()),
                interface_id: Some("00".into()),
                manufacturer_contains: Some("Example Devices".into()),
                name_contains: Some("회사 키보드".into()),
