...
impl DeviceSelector {
    pub fn is_empty(&self) -> bool {
        self.vendor_id.is_none()
            ...
            && self.manufacturer_contains.is_none()
            && self.name_contains.is_none()
            && self.is_virtual.is_none()
    }

    pub fn specificity(&self) -> usize {
        usize::from(self.vendor_id.is_some())
            + usize::from(self.product_id.is_some())
            + usize::from(self.interface_id.is_some())
            + usize::from(self.manufacturer_contains.is_some())
            + usize::from(self.name_contains.is_some())
            + usize::from(self.is_virtual.is_some())
    }
}
