...
KeyForge 0.1.9 intentionally focuses on stable global key and mouse remapping.

- Global single-gesture keyboard and mouse remaps are connected to native
  low-level hooks and `SendInput`.
- Profiles can now auto-activate when matching keyboards are connected, but this
  is based on the current connected-device set, not the source device that
  generated an individual key press.
...
- The read-only Devices tab lists only keyboard endpoints returned by Windows
  ...
  not persistent per-device remap IDs. The new activation selectors intentionally
  store only stable fragments such as VID/PID/MI, manufacturer/name contains,
  and virtual-device state, so identical models are still grouped together.
