...
describe('browser mock bridge', () => {
  ...
  it('bootstraps a fully usable global-first settings model', async () => {
    ...
    expect(payload.settings.profiles[0].activation.connectedKeyboards).toEqual([]);
  ...
  it('round-trips connected keyboard activation and recomputes active profile count', async () => {
    const initial = await keyforgeBridge.bootstrap();
    const activated = makeProfile('회사 프로필');
    activated.activation.connectedKeyboards = [{
      vendorId: '046D',
      productId: 'C31C',
      interfaceId: '00',
      manufacturerContains: 'Example Devices',
      nameContains: '테스트 기계식 키보드',
      isVirtual: false,
    }];

    const saved = await keyforgeBridge.saveSettings(
      { ...initial.settings, profiles: [...initial.settings.profiles, activated] },
      initial.settings.revision,
    );
    const reloaded = await keyforgeBridge.bootstrap();

    expect(saved.settings.profiles.at(-1)?.activation.connectedKeyboards).toEqual(activated.activation.connectedKeyboards);
    expect(reloaded.runtime.activeProfileCount).toBe(3);
  });
