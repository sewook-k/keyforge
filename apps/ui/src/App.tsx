...
const responsePromise = keyforgeBridge.saveSettings(draft, previous.revision);
      ...
      const connectedKeyboards = await keyforgeBridge.listConnectedKeyboards().catch(() => []);
      setBootstrap((current) => current ? {
        ...current,
        settings,
        runtime: {
          ...current.runtime,
          activeProfileCount: countActiveProfiles(settings, connectedKeyboards),
        },
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="topbar__brand"><IconButton label="메뉴 열기" className="mobile-menu" onClick={() => setSidebarOpen(true)}><Menu size={20} /></IconButton><div className="brand-mark">K</div><strong>KeyForge</strong><span className="version-badge">0.1</span></div>
